...
export default function ConsultasPage() {
  ...
  const load = (v: 'minha' | 'todos' = view) => {
    setLoading(true)
    const consultas = v === 'minha' && usuario
      ? consultasApi.buscarPorProfissional(usuario.id)
      : consultasApi.listar()
    Promise.all([consultas, pacientesApi.listar(), profissionaisApi.listar()])
  ...
  const trocarView = (v: 'minha' | 'todos') => {
    setView(v)
    load(v)
  }
  ...
  return (
    <div>
        ...
        <div style={{ display: 'flex', gap: 8, marginBottom: 20 }}>
          {(['minha', 'todos'] as const).map(v => (
            <button key={v} onClick={() => trocarView(v)} style={{
              ...
              {v === 'minha' ? 'Minhas consultas' : 'Todas as consultas'}
            </button>
          ))}
        </div>
      )}
      ...
      {loading ? <Loading /> : lista.length === 0 ? (
        <Empty message={view === 'minha' ? 'Você ainda não registrou consultas.' : 'Nenhuma consulta registrada.'} />
      ) : (
        <div style={{ display: 'flex', flexDirection: 'column', gap: 10 }}>
          {lista.map(c => (
            <Card key={c.id} style={{ display: 'flex', flexWrap: 'wrap', alignItems: 'flex-start', justifyContent: 'space-between', gap: 16 }}>
              <div style={{ flex: 1, minWidth: 0 }}>
                <div style={{ display: 'flex', flexWrap: 'wrap', gap: 12, alignItems: 'center', marginBottom: 6 }}>
                  ...
                  <span style={{ fontSize: 11.5, color: 'var(--muted)', fontVariantNumeric: 'tabular-nums', marginLeft: 'auto' }}>
              ...
              <div style={{ display: 'flex', gap: 6, flexShrink: 0 }}>
                ...
                <Button size="sm" variant="danger" onClick={() => handleDelete(c.id)}><X size={14} /></Button>
              </div>
            </Card>
          ))}
        </div>
