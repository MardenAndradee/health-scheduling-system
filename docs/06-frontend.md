...
3. **Cálculo de urgência agora é decidido no backend, mas os pesos por especialidade ainda são um placeholder.** O wizard do paciente (`app/(paciente)/portal/solicitar/page.tsx`) não calcula mais o nível — envia `especialidadeId`/`idade`/`respostas` brutos e o backend resolve via `AnamneseService.defineUrgencia` (ver [API REST](04-api-rest.md#anamneses-triagem--anamneses)). Cada método por especialidade (`calcularUrgencia<Especialidade>`) já recebe as respostas extraídas e tipadas, mas ainda retorna sempre `VERDE` — a lógica de peso está marcada com `// TODO` no código, pendente de implementação. `lib/especialidades/pontuacao.ts` (o cálculo antigo, client-side) ficou sem nenhuma referência no código, mas não foi removido.
